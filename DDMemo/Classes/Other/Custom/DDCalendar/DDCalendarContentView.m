...
@interface DDCalendarContentView()<UICollectionViewDataSource,UICollectionViewDelegate>{
    //是否是在点击日期或者滑动改变页数
...
@implementation DDCalendarContentView
...
- (void)initUI {
    self.flowLayout = [DDCalendarCollectionViewFlowLayout new];
    self.flowLayout.itemSize = CGSizeMake(self.frame.size.width/7, [DDCalendarAppearance share].weekDayHeight);
    self.flowLayout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
    self.flowLayout.itemCountPerRow = 7;
    self.flowLayout.rowCount = [DDCalendarAppearance share].weeksToDisplay;
    ...
    [self.collectionView registerClass:[DDCalendarCollectionCell class] forCellWithReuseIdentifier:@"dayCell"];
    self.backgroundColor = [DDCalendarAppearance share].calendarBgColor;
    
    ///先初始化数据
    [self getDateDatas];
    
    UIView *maskView = [[UIView alloc] initWithFrame:self.bounds];
    maskView.backgroundColor = [DDCalendarAppearance share].calendarBgColor;
    maskView.alpha = 0;
...
- (void)setSingleWeek:(BOOL)singleWeek{
    [DDCalendarAppearance share].isShowSingleWeek = singleWeek;
    //    self.flowLayout.rowCount = singleWeek ? 1:[DDCalendarAppearance share].weeksToDisplay;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    DDCalendarCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"dayCell" forIndexPath:indexPath];
    DDCalendarDayItem *item = self.daysInMonth[indexPath.section][indexPath.row];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    DDCalendarCollectionCell *cell = (DDCalendarCollectionCell*)[collectionView cellForItemAtIndexPath:indexPath];
    cell.isSelected = true;
    DDCalendarDayItem *itemCurrent;
    DDCalendarDayItem *itemLast;
    NSArray *dataSource;
    if ([DDCalendarAppearance share].isShowSingleWeek) {
    ...
    NSDateComponents *comps = [[DDCalendarAppearance share].calendar components:NSCalendarUnitMonth | NSCalendarUnitWeekOfMonth | NSCalendarUnitWeekday fromDate:itemCurrent.date];
    ...
    if (touchMonthIndex == currentMonth || [DDCalendarAppearance share].isShowSingleWeek) {
        DDCalendarCollectionCell *lastCell = (DDCalendarCollectionCell*)[collectionView cellForItemAtIndexPath:self.currentSelectedIndexPath];
        lastCell.isSelected = false;
    }
    
    NSInteger index = comps.weekday-[DDCalendarAppearance share].calendar.firstWeekday%7;
    if (index < 0) {
        index += 7;
    }
...
- (void)getDateDatas{
    ...
    if (self.currentDate == nil) {
        self.currentDate = [DDCalendarAppearance share].defaultDate;
    }
...
- (NSDate *)getNewCurrentDate{
    ...
    if (currentPage == round(NUMBER_PAGES_LOADED / 2) && [DDCalendarAppearance share].isShowSingleWeek){
...
- (CGFloat)singleWeekOffsetY{
    return  self.currentSelectedIndexPath.row/7*[DDCalendarAppearance share].weekDayHeight;
}
...
- (NSDate *)beginningOfWeek:(NSDate *)date{
    ...
    NSInteger index = componentsCurrentDate.weekday-[DDCalendarAppearance share].calendar.firstWeekday%7;
    if (index < 0) {
        index += 7;
    }
...
- (NSArray *)getDaysOfWeek:(NSDate *)date
{
    ...
    for (NSInteger i=0; i<7; i++) {
        ...
        DDCalendarDayItem *item = [DDCalendarDayItem new];
...
- (NSArray*)getDaysOfMonth:(NSDate *)date
{
    ...
    for (NSInteger i = 0; i<[DDCalendarAppearance share].weeksToDisplay; i++) {
        NSDateComponents *dayComponent = [NSDateComponents new];
        dayComponent.day = 7;
